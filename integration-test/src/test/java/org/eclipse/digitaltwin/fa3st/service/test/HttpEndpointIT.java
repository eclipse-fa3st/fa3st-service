...
import static org.eclipse.digitaltwin.fa3st.service.test.util.MessageBusHelper.DEFAULT_TIMEOUT;
import static org.eclipse.digitaltwin.fa3st.service.test.util.MessageBusHelper.assertEvent;
import static org.eclipse.digitaltwin.fa3st.service.test.util.MessageBusHelper.assertEvents;
...
import org.eclipse.digitaltwin.fa3st.common.util.ExtendHelper;
...
import org.eclipse.digitaltwin.fa3st.service.persistence.util.QueryModifierHelper;
import org.eclipse.digitaltwin.fa3st.service.request.RequestHandlerManager;
import org.eclipse.digitaltwin.fa3st.service.request.handler.StaticRequestExecutionContext;
...
public class HttpEndpointIT extends AbstractIntegrationTest {
    ...
    public void init() throws Exception {
        ...
        operationSquare = (Operation) AASFull.getFa3stSubmodel(environment)
    ...
    public void testAASBasicDiscoveryGetAssetAdministrationShellsByGlobalAssetId()
        ...
        assertExecutePage(
                HttpMethod.GET,
                apiPaths.aasBasicDiscovery().assetAdministrationShells(Map.of(Fa3stConstants.KEY_GLOBAL_ASSET_ID, assetIdValue)),
                StatusCode.SUCCESS,
                null,
                expected,
                String.class);
        ...
        assertEvent(
                ...
                LambdaExceptionHelper.wrap(
                        x -> {
                            ...
                            Assert.assertEquals(toHttpStatusCode(StatusCode.SUCCESS), response.statusCode());
                            assertJsonEquals(expected, response.body());
                        }));
    }
        ...
        assertEvent(
                ...
                LambdaExceptionHelper.wrap(
                        x -> {
                            ...
                            Assert.assertEquals(toHttpStatusCode(StatusCode.SUCCESS), response.statusCode());
                            assertJsonEquals(mapper.writeValueAsString(expected.getPaths()), response.body());
                        }));
    }
        ...
        assertEvent(
                ...
                LambdaExceptionHelper.wrap(
                        x -> {
                            ...
                            Assert.assertEquals(toHttpStatusCode(StatusCode.SUCCESS), response.statusCode());
                            assertJsonEquals(mapper.writeValueAsString(expected.asCorePath().getPaths()), response.body());
                        }));
    }
    ...
    public void testSubmodelInterfaceInvokeOperationAsyncValueOnly()
            throws IOException, DeserializationException, InterruptedException, URISyntaxException, SerializationException, MessageBusException, NoSuchAlgorithmException,
            KeyManagementException, UnsupportedContentModifierException, UnsupportedModifierException {
        int inputValue = 4;
        ...
        assertJsonEquals(expectedPayload, response.body().toString());
    }
